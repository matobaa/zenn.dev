---
title: " バイナリとTLS鍵・証明書を準備する"
---

作業用の`jumpbox`コンテナに入り、各ノードで必要となるバイナリを入手します。
/root/control にコントロールプレーン用、/root/worker にワーカーノード用のバイナリを集めておき、あとからそれを配布します。
...
curl https://storage.googleapis.com/etcd/v3.6.5/etcd-v3.6.5-linux-arm64.tar.gz | tar xzf -
install -m 755 etcd-v3.6.5-linux-arm64/etcd* /root/control/usr/local/bin/
install -m 755 etcd-v3.6.5-linux-arm64/etcd* /usr/local/bin/

# download and extract the kubernetes binaries
# ref: https://kubernetes.io/releases/download/#binaries

wget -q --show-progress \
  https://dl.k8s.io/v1.34.1/bin/linux/arm64/kube-apiserver \
  https://dl.k8s.io/v1.34.1/bin/linux/arm64/kube-controller-manager \
  https://dl.k8s.io/v1.34.1/bin/linux/arm64/kube-scheduler \
  https://dl.k8s.io/v1.34.1/bin/linux/arm64/kubectl \
  https://dl.k8s.io/v1.34.1/bin/linux/arm64/kube-proxy \
  https://dl.k8s.io/v1.34.1/bin/linux/arm64/kubelet
...
ここからは、以下のコンポーネント間で安全に通信するための、TLSの鍵と証明書を準備します。
図中の矢印の先にはサーバ証明書と秘密鍵、矢印の元にはクライアント証明書と秘密鍵、またそれぞれの場所には証明書を検証するためのCA証明書を配置します。ETCD関連の証明書はETCD-CAが、k8s関連の証明書はkubenetes-caが発行者(Issuer)となります。

![](/images/kinaco_thehardway/kubernetes-pki.drawio.png)
**図: コンポーネント間の接続図**

[ベストプラクティス](https://kubernetes.io/docs/setup/best-practices/certificates/)に従い、TLS関連のファイルを `/etc/kubernetes/pki/`以下に作成していきます:

| Path.{key,crt}            | Subject:/CN=                  | Issuer CA     | Usage
|---                        |---                            |---            |---
...
管理者用・コンポーネント用の証明書は `/etc/kubernetes/*.conf`に格納していきますが、
ここでは上記同様、ひとまず .key と .crtを `/etc/kubernetes/pki/` に格納しておきます。

| Path.{key,crt}            | Subject                                   | Issuer CA     | Usage
|---                        |---                                        |---            |---
|./admin                    | /CN=admin/O=<admin-group>                 | kubernetes-ca | client
|./super-admin              | /CN=<any>/O=system:masters                | kubernetes-ca | client
|./kubelet                  | /CN=system:node:<nodeName>/O=system:nodes | kubernetes-ca | client
|./controller-manager       | /CN=system:kube-controller-manager        | kubernetes-ca | client
|./scheduler                | /CN=system:kube-scheduler                 | kubernetes-ca | client
|./kube-proxy               | /CN=system:kube-proxy                     | kubernetes-ca | client

NOTE: 
- TLSを利用するサーバ証明書には、SAN属性が必要です。
- RBACを利用するクライアント証明書は、Subjectに /CN や /O を適切に含む必要があります。
- Node認証を利用するkubelet証明書は、Subjectが system:node:ホスト名 である必要があります。
...
openssl x509 -req -in apiserver.csr -CA ca.crt -CAkey ca.key -days 365 -out apiserver.crt -extfile - <<-EOF
  ...
  subjectAltName=DNS:control-0.internal,DNS:control-1.internal,DNS:control-2.internal,IP:172.20.0.1,DNS:kubernetes,DNS:kubernetes.default
EOF
...
etcdに格納するsecretを暗号化/復号するための共有鍵を作ります。暗号化プロバイダはここではAES-CBCを指定しておきます。
なお、CBCのパディングオラクル攻撃が知られていること、ControlPlane内で参照可能であることから非推奨です。
https://kubernetes.io/docs/tasks/administer-cluster/encrypt-data/

pki証明書のディレクトリを間借りしてファイルを置いておきます。

```shell:jumpbox

<<EOF sed -e "/^ *#/d" > /etc/kubernetes/pki/encryption-config.yaml
...
container exec jumpbox tar cf - etc/kubernetes/pki \
   | tar xf -
```